...
**Before you bet a signal you found**: `trading_audit_signal` (six gates: sample, significance, multiple testing, out-of-sample, CLV, execution cost → CANDIDATE / WATCH / NO SIGNAL) and `trading_testing_bar` (what a search of N hypotheses costs in evidence). Register every hypothesis BEFORE testing it with `npm run budget -- register` — it issues a `research_token`, and without one the multiple-testing gate stays shut and nothing can reach CANDIDATE. With one, the count is read off the ledger instead of taken from the caller. See docs/The-Judge.md — the repo's own best finding scores NO SIGNAL.
