...
const PROVIDERS: Record<string, () => Promise<{ register: (s: McpServer) => void }>> = {
  ...
  polymarket:     () => import("./providers/polymarket.js"),
...
//
//   Not set / empty    → load "free" preset (21 providers, 183 tools)
//   "all"              → load ALL 44 providers (430 tools)
...
//   "cricket", "golf", "chess", and "free" (all 21 no-key providers — the default).
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  if (isAll) {
    console.error("");
    console.error(`  ⚠ All ${Object.keys(PROVIDERS).length} providers loaded (430 tools).`);
    console.error("    LLMs work best with fewer tools. Consider using a preset:");
    console.error("    SPORTS_HUB_PROVIDERS=free        → 21 providers, 183 tools (no keys needed)");
