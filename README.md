...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tools-428-green" alt="428 Tools">
...
A unified MCP server that aggregates **44 sports API providers** into a single service. **428 tools** covering scores, stats, odds, esports, college sports, chess, motorsport, boxing, AFL, betting/trading maths, and more across 70+ sports.
...
| `polymarket_` | [Polymarket](https://polymarket.com/) | Prediction-market prices, order books, cross-venue comparison | 5 | Keyless reads; written from the API shape, unverified |
...
> **Tip**: Use `SPORTS_HUB_PROVIDERS=free` to load only these 21 providers (181 tools).
...
By default, only the **free preset** is loaded (21 providers, 181 tools — no API keys needed). Use `SPORTS_HUB_PROVIDERS` to change what's loaded:

```bash
# Default — free providers only (no config needed)
npx mcp-sports-hub

# Load ALL 44 providers (428 tools)
...
|--------|-----------|-------|-------------|
| `free` (default) | 21 no-key providers (espn, nhl, mlb, f1, openf1, openliga, sportsdb, ncaa, sportsrc, lichess, chesscom, squiggle, motogp, formulae, nascar, opendota, sleeper, euroleague, footballdatauk, trading, polymarket) | 181 | No |
| `all` | all 44 providers | 428 | Yes (for key-required providers) |
...
| `odds` | odds, oddsio, sgo, lumify, trading, polymarket | 61 | Some |
| `trading` | trading, polymarket, footballdatauk, odds, oddsio, sgo, lumify, apifootball, footballdata | 87 | Some |
...
**Prompts** (curated slash-command workflows over the 428 tools):
...
└── providers/
    ...
    ├── polymarket.ts           #   5 tools — no key (prediction market)
