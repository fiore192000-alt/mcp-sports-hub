...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tools-430-green" alt="430 Tools">
...
A unified MCP server that aggregates **44 sports API providers** into a single service. **430 tools** covering scores, stats, odds, esports, college sports, chess, motorsport, boxing, AFL, betting/trading maths, and more across 70+ sports.
...
### Works instantly — no API key, no signup (21 providers, 183 tools)
...
> **Tip**: Use `SPORTS_HUB_PROVIDERS=free` to load only these 21 providers (183 tools).
...
By default, only the **free preset** is loaded (21 providers, 183 tools — no API keys needed). Use `SPORTS_HUB_PROVIDERS` to change what's loaded:

```bash
# Default — free providers only (no config needed)
npx mcp-sports-hub

# Load ALL 44 providers (430 tools)
...
| `all` | all 44 providers | 430 | Yes (for key-required providers) |
...
**Prompts** (curated slash-command workflows over the 430 tools):
...
└── providers/
    ...
    ├── trading.ts              #  13 tools — no key (local maths + CSV archive)
