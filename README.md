...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tools-421-green" alt="421 Tools">
...
A unified MCP server that aggregates **43 sports API providers** into a single service. **421 tools** covering scores, stats, odds, esports, college sports, chess, motorsport, boxing, AFL, betting/trading maths, and more across 70+ sports.
...
### Works instantly — no API key, no signup (20 providers, 174 tools)
...
| `trading_` | Trading toolkit (local) | Betting maths + match prediction: de-vig, EV/Kelly, arbitrage, hedging, Poisson model, fixture predictions, forecast scoring, backtest | 11 | Pure computation + the football-data.co.uk archive |

> **Tip**: Use `SPORTS_HUB_PROVIDERS=free` to load only these 20 providers (174 tools).
...
By default, only the **free preset** is loaded (20 providers, 174 tools — no API keys needed). Use `SPORTS_HUB_PROVIDERS` to change what's loaded:

```bash
# Default — free providers only (no config needed)
npx mcp-sports-hub

# Load ALL 43 providers (421 tools)
...
|--------|-----------|-------|-------------|
| `free` (default) | 20 no-key providers (espn, nhl, mlb, f1, openf1, openliga, sportsdb, ncaa, sportsrc, lichess, chesscom, squiggle, motogp, formulae, nascar, opendota, sleeper, euroleague, footballdatauk, trading) | 174 | No |
| `all` | all 43 providers | 421 | Yes (for key-required providers) |
| `chess` | lichess, chesscom | 14 | No |
| `us-major` | espn, nhl, mlb, ncaa, cfbd, bdl, msf, nascar, sleeper | ~93 | Some |
| `soccer` | espn, apifootball, footballdata, sportmonks, openliga, sportsrc, footballdatauk, highlightly, trading | 82 | Some |
| `f1` | f1, openf1 | 25 | No |
| `motorsport` | f1, openf1, motogp, formulae, nascar | ~42 | No |
| `esports` | pandascore, opendota | 25 | Some |
| `odds` | odds, oddsio, sgo, lumify, trading | 54 | Some |
| `trading` | trading, footballdatauk, odds, oddsio, sgo, lumify, apifootball, footballdata | 80 | Some |
...
└── providers/
    ...
    ├── trading.ts              #  11 tools — no key (local maths + CSV archive)
