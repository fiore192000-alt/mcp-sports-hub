...
- Keys are checked at **call time**, not at startup. The server registers whichever providers are selected (the `free` preset by default — 20 providers / 174 tools — or all 43 providers / 421 tools with `SPORTS_HUB_PROVIDERS=all`). If you call a tool that requires a missing key, you get an error message like `"API_FOOTBALL_KEY env var is required"`.
