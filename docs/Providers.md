...
Sports Hub aggregates **43 independent sports API providers** into one MCP server. Each provider registers its own tools, all prefixed with the provider name to avoid collisions. **419 tools** total.
...
**Tools:** `trading_list_strategies`, `trading_devig_odds`, `trading_evaluate_bet`, `trading_find_arbitrage`, `trading_hedge_position`, `trading_poisson_model`, `trading_team_ratings`, `trading_backtest`, `trading_closing_line_value`
